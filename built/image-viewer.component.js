...
var image_viewer_style_2 = require("./image-viewer.style");
var ImageViewer = /** @class */ (function (_super) {
    ...
    function ImageViewer() {
        ...
        _this.goBack = function () {
            ...
            var width = _this.state.containerDimensions.width;
            _this.positionXNumber = !react_native_1.I18nManager.isRTL ? _this.standardPositionX + width : _this.standardPositionX - width;
        ...
        _this.handleLayout = function (event) {
            ...
            var _b = _this.state.containerDimensions, prevWidth = _b.width, prevHeight = _b.height;
            if (width !== prevWidth || height !== prevHeight) {
                var _c = event.nativeEvent.layout, width_1 = _c.width, height_1 = _c.height;
                _this.setState({ containerDimensions: { width: width_1, height: height_1 } });
                // 强制刷新
        ...
        _this.styles = function () {
            ...
            var _a = _this.state.containerDimensions, width = _a.width, height = _a.height;
            return image_viewer_style_1.default(width, height, backgroundColor);
        };
        /**
         * 保存当前图片到本地相册
         */
    ...
    ImageViewer.prototype.jumpToCurrentImage = function () {
        ...
        var newPositionXNumber = width * (this.state.currentShowIndex || 0) * (react_native_1.I18nManager.isRTL ? 1 : -1);
        ...
        if (newPositionXNumber === this.positionXNumber)
            return;
    ...
    ImageViewer.prototype.getContent = function () {
        var _this = this;
        // 获得屏幕宽高
        var _a = this.state.containerDimensions, screenWidth = _a.width, screenHeight = _a.height;
        ...
        var ImageElements = this.props.imageUrls.map(function (image, index) {
            ...
            var imageSizes = _this.state.imageSizes;
            var imageInfo = imageSizes && imageSizes[index];
            ...
            var Wrapper = function (_a) {
                ...
                return (<react_native_image_pan_zoom_1.default cropWidth={screenWidth} cropHeight={screenHeight} maxOverflow={_this.props.maxOverflow} horizontalOuterRangeOffset={_this.handleHorizontalOuterRangeOffset} responderRelease={_this.handleResponderRelease} onMove={_this.props.onMove} onLongPress={_this.handleLongPressWithIndex.get(index)} onClick={_this.handleClick} onDoubleClick={_this.handleDoubleClick} enableSwipeDown={_this.props.enableSwipeDown} swipeDownThreshold={_this.props.swipeDownThreshold} onSwipeDown={_this.handleSwipeDown} pinchToZoom={_this.props.enableImageZoom} enableDoubleClickZoom={_this.props.enableImageZoom} doubleClickInterval={_this.props.doubleClickInterval} useNativeDriver={_this.props.useNativeDriver} {...others}>
          {children}
        </react_native_image_pan_zoom_1.default>);
            };
            switch (imageInfo.status) {
                case 'loading':
                    return (<Wrapper key={index} style={react_native_1.StyleSheet.flatten([styles.modalContainer, styles.loadingContainer])} imageWidth={screenWidth} imageHeight={screenHeight}>
              <react_native_1.View style={styles.loadingContainer}>{_this.props.loadingRender()}</react_native_1.View>
            </Wrapper>);
                case 'success':
                    if (!image.props) {
                        image.props = {};
                    ...
                    var source = image.props.source || {};
                    if (typeof source !== 'number') {
                        // source = require(...) if it's a number - we don't transform it
                        source = __assign({ uri: image.url }, source);
                    }
                    if (_this.props.enablePreload) {
                        _this.preloadImage(_this.state.currentShowIndex || 0);
                    }
                    return (<react_native_image_pan_zoom_1.default key={index} cropWidth={screenWidth} cropHeight={screenHeight} maxOverflow={_this.props.maxOverflow} horizontalOuterRangeOffset={_this.handleHorizontalOuterRangeOffset} responderRelease={_this.handleResponderRelease} onMove={_this.props.onMove} onLongPress={_this.handleLongPressWithIndex.get(index)} onClick={_this.handleClick} onDoubleClick={_this.handleDoubleClick} imageWidth={width} imageHeight={height} enableSwipeDown={_this.props.enableSwipeDown} swipeDownThreshold={_this.props.swipeDownThreshold} onSwipeDown={_this.handleSwipeDown} panToMove={!_this.state.isShowMenu} pinchToZoom={_this.props.enableImageZoom && !_this.state.isShowMenu} enableDoubleClickZoom={_this.props.enableImageZoom && !_this.state.isShowMenu} doubleClickInterval={_this.props.doubleClickInterval} minScale={_this.props.minScale} maxScale={_this.props.maxScale} useNativeDriver={_this.props.useNativeDriver}>
              {_this.props.renderImage(__assign(__assign({}, image.props), { style: style, source: source }))}
            </react_native_image_pan_zoom_1.default>);
                case 'fail':
                    return (<Wrapper key={index} style={react_native_1.StyleSheet.flatten(styles.modalContainer)} imageWidth={_this.props.failImageSource ? _this.props.failImageSource.width : screenWidth} imageHeight={_this.props.failImageSource ? _this.props.failImageSource.height : screenHeight}>
        ...
        var _b = this.props, renderHeader = _b.renderHeader, renderArrowLeft = _b.renderArrowLeft, renderArrowRight = _b.renderArrowRight;
        return (<react_native_1.Animated.View style={{ zIndex: 9 }}>
        <react_native_1.Animated.View style={[styles.container, { opacity: this.fadeAnim }]}>
          {renderHeader ? renderHeader(this.state.currentShowIndex) : null}

          <react_native_1.View style={styles.arrowLeftContainer}>
            <react_native_1.TouchableWithoutFeedback onPress={this.goBack}>
              <react_native_1.View>{renderArrowLeft ? renderArrowLeft() : null}</react_native_1.View>
            </react_native_1.TouchableWithoutFeedback>
          </react_native_1.View>

          <react_native_1.View style={styles.arrowRightContainer}>
            <react_native_1.TouchableWithoutFeedback onPress={this.goNext}>
              <react_native_1.View>{renderArrowRight ? renderArrowRight() : null}</react_native_1.View>
            </react_native_1.TouchableWithoutFeedback>
          </react_native_1.View>
          ...
          {this.props.imageUrls[this.state.currentShowIndex || 0] &&
            ...
            this.props.imageUrls[this.state.currentShowIndex || 0].originUrl && (<react_native_1.View style={styles.watchOrigin}>
                <react_native_1.TouchableOpacity style={styles.watchOriginTouchable}>
                  <react_native_1.Text style={styles.watchOriginText}>查看原图(2M)</react_native_1.Text>
    ...
    ImageViewer.prototype.getMenu = function () {
        ...
        var styles = this.styles();
        if (this.props.menus) {
            return (<react_native_1.View style={styles.menuContainer}>
          {this.props.menus({ cancel: this.handleLeaveMenu, saveToLocal: this.saveToLocal })}
        </react_native_1.View>);
        }
        return (<react_native_1.View style={styles.menuContainer}>
        <react_native_1.View style={styles.menuShadow}/>
        <react_native_1.View style={styles.menuContent}>
          <react_native_1.TouchableHighlight underlayColor="#F2F2F2" onPress={this.saveToLocal} style={styles.operateContainer}>
            <react_native_1.Text style={styles.operateText}>{this.props.menuContext.saveToLocal}</react_native_1.Text>
          </react_native_1.TouchableHighlight>
          <react_native_1.TouchableHighlight underlayColor="#F2F2F2" onPress={this.handleLeaveMenu} style={styles.operateContainer}>
            <react_native_1.Text style={styles.operateText}>{this.props.menuContext.cancel}</react_native_1.Text>
          </react_native_1.TouchableHighlight>
        </react_native_1.View>
      </react_native_1.View>);
    };
    ImageViewer.prototype.render = function () {
        var childs = (<react_native_1.View>
        {this.getContent()}
        {this.getMenu()}
      </react_native_1.View>);
    ...
    ImageViewer.defaultProps = {
        ...
        renderIndicator: function (currentIndex, allSize) {
            return React.createElement(react_native_1.View, { style: image_viewer_style_2.simpleStyle.count }, React.createElement(react_native_1.Text, { style: image_viewer_style_2.simpleStyle.countText }, currentIndex + '/' + allSize));
        },
        // eslint-disable-next-line react/display-name
